...
simplify = map simpl 
  where 
    simpl (i:>:(ot, Forall (k:=>t))) = let (k':=>t') = rem k:=>t in {- (i:>:(ot, Forall (rem k':=>t'))) -}
...
satAssump g0 owi a@(i:>:(ot, Forall t)) = do let k = getK' t
                                             if null k then return a
                                                else
                                                 do 
                                                    s'  <- sat (isOver g0) owi i t 
                                                    so  <- closeOWassump (isOver g0) owi i t
                                                    -- *** ALTERAR verificar projecao de restricoes e se for o caso fechar o mundo.
                                                    return (i:>:(ot, Forall (apply (so@@s') t)))



closeOWassump g0 owi i kt@(k:=>t) = do let k' = k |* tv t
	   			           ck = k \\ k'   
				       if null ck then return nullSubst 
                                        else
                                          do
                                             let owi' = owi \\ (map (\(Constraint (i, _, _)) -> i) ck)
                                             s <- sat g0 owi' i kt -- Eh necessario executar sat apenas para as restricoes em ck
                                             return s
...
satDrive g0 g = do g0'<- mapM freshA g0
                   ...
                   --gr <- remSelfC g'  
                   let ga  = g' `union` g0'
		       iow = map (\(i :>: _) -> i) (filter open_world ga)
                   g''  <- {-# SCC "SAT" #-} (mapM (satAssump ga iow) g')
                   return g''     
...
sat g0 owi i kt@(k:=>t) = do let kcw = filter (nElemConst owi) k
				 kps = constraintP kcw
                             rs <- mapM (\k -> (satsO (kcw, g0, [], 3))) kps
                             if any null rs then error ("Function " ++ show i ++ ": Constraint\n" ++ impConstraint k ++ "\nis not satisfiable")
                                  ...
                                  do 
                                     ...
                                     return (concat ss)
   where
      nElemConst owi (Constraint (i, _, _)) = not (elem i owi) 
